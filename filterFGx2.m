function [filtdat,empVals] = filterFGx2(data,srate,f,fwhm,showplot) 
% 10/11/17 version : changed line 51: use size instead of length
% to avoid hz taking the length of the data, because when there are more
% trials than timepoints length data returns the number of trials. duprez.joan@gmail.com
...
% frequencies
hz = linspace(0,srate,size(data,2)); % changed length(data) to size(data,2) to be sure to have the number of timepoints in case of trials>timepoints in data
